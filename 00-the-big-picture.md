...
- `api/core/app/apps/` shows how every app mode converges on the shared execution path.
- `api/core/workflow/workflow_entry.py`, `api/core/workflow/node_factory.py`, and `api/core/workflow/nodes/` form the Dify side of the engine boundary.
- `api/celery_entrypoint.py`, `api/tasks/`, and `api/core/app/apps/base_app_queue_manager.py` carry background execution and event streaming.
