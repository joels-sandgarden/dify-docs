...
The same facade serves the main model-facing surfaces across the product. Workflow nodes use the quota layer in `api/core/app/workflow/layers/llm_quota.py`, LLM-backed agents and utility generation call through `ModelInstance`, and RAG indexing and query-time paths resolve embeddings and reranking through the same tenant-bound manager. The code keeps one model abstraction for all of those surfaces, so provider selection and credential handling stay consistent regardless of where the request starts. For the graph engine boundary that drives those nodes, see [Inside the graph engine](/02-the-graph-engine.md).
...
- graphon: `src/graphon/model_runtime/README.md`
