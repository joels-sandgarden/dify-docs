...
That boundary matters because the API process does not execute plugin code itself. It treats the daemon as the execution surface for plugin behavior and only consumes typed responses, streamed events, and error envelopes. The repository does not include the daemon implementation, so the chapter stops at the client side and the deployment surface that the code makes visible.

## Execution environments

The compose file and env example show the execution surfaces that the repository can prove. The daemon listens on `PLUGIN_DAEMON_PORT` with a default of `5002`, exposes a remote install and debug port on `PLUGIN_DEBUGGING_PORT` with a default of `5003`, and reads its inner API address and key from the environment. The API and worker services point at the daemon through `PLUGIN_DAEMON_URL` and `PLUGIN_DAEMON_KEY`, while the agent backend points at the same service through `DIFY_AGENT_PLUGIN_DAEMON_URL` and `DIFY_AGENT_PLUGIN_DAEMON_API_KEY`. Dify’s public runtime material describes local process, debug connection, and hosted operation as part of the same plugin story; this repository only proves the container and environment wiring behind that split.
