...
`Dataset`, `Document`, and `DocumentSegment` form the core storage model. `Dataset` carries tenant, retrieval, indexing, and pipeline metadata; it does not carry chunk text or vectors. `Document` records one source item plus its lifecycle timestamps from parsing through completion, and it keeps the source, process rule, and document shape attached to that item. `DocumentSegment` carries the chunk text itself, the segment status, hit counts, and the vector index node identity that retrieval uses later. `ChildChunk` narrows that same pattern for hierarchical indexing: it stores smaller child text under a parent segment so retrieval can work on smaller units without losing the parent container. `DatasetProcessRule` stores the segmentation rule set, `DatasetKeywordTable` stores the keyword sidecar used by keyword search, `Pipeline` binds a dataset to a workflow pipeline, and `DocumentPipelineExecutionLog` records each document run through that pipeline.
...
- `api/core/rag/index_processor/processor/paragraph_index_processor.py` and `api/core/rag/index_processor/processor/parent_child_index_processor.py` — chunking, keyword path, vector path, and parent child tradeoffs
...
- `api/core/workflow/nodes/knowledge_retrieval/knowledge_retrieval_node.py` and `api/core/tools/utils/dataset_retriever/` — app and tool integration